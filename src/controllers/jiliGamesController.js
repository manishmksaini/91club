...
const gameCategoriesPage = GameCategoriesId => async (req, res) => {
   try {
      ...
      const slotsGame = response?.data?.gameData.find(item => item.id === GameCategoriesId)
...
const gameList = async (req, res) => {
   try {
      ...
      return res.status(401).json({ errorCode: 4, message: "Error while fetching from database!" })
   }
}
...
const auth = async (req, res) => {
   try {
      ...
      if (!reqId || !token) {
         return res.status(400).json({ errorCode: 5, message: "Request ID and token are required" })
      }

      const [rows] = await connection.execute("SELECT * FROM `users` WHERE `token` = ? AND `veri` = 1", [token])
      ...
      return res.status(401).json({ errorCode: 4, message: "Token expired or invalid" })
   } catch (error) {
      console.log(error)
      return res.status(500).json({ errorCode: 4, message: "Error while fetching from database!" })
   }
}
...
const bet = async (req, res) => {
   ...
   try {
      ...
      // Check if reqId and token are provided
      if (!reqId || !token) {
         return res.status(400).json({ errorCode: 4, message: "Request ID and token are required" })
      }
      ...
      if (rows[0]?.status == 1) {
         ...
         await connection.query("UPDATE users SET money = ?, total_money = ? WHERE `phone` = ?", [finalAmount, finalAmount, username])
...
const cancelBet = async (req, res) => {
   ...
   try {
      ...
      console.log(error)
      return res.status(500).json({ errorCode: 5, message: "Error while fetching from database!" })
   }
}
   ...
   try {
      ...
      if (rows[0]?.status == 1) {
         ...
         if (preserve === 0 && betAmount > 0 && winloseAmount === 0) {
            finalAmount = balance - betAmount
         } else if (preserve === 0 && betAmount === 0 && winloseAmount >= 0) {
            finalAmount = balance + winloseAmount
         } else if ((preserve > 0, betAmount === 0, winloseAmount === 0)) {
            finalAmount = balance - preserve
         } else if (preserve > 0 && betAmount >= 0 && winloseAmount >= 0) {
